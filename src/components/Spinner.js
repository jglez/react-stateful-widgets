...
STEP 1:
  Create a 'spinnerOn', 'setSpinnerOn' pair of variables using the state hook. ------------------------------------------------> DONE
...
STEP 2:
  ...
  the one on the right becomes the value of the whole line. If an expression on either side of the '&&' -----------------------> DONE
...
STEP 3:
  Use a ternary expression inside the text of the button, to render "Hide" or "Show" depending on the value of 'spinnerOn'. ---> DONE

STEP 4:
  This click handler needs to toggle the spinner by setting "whether on" to be the opposite of what it currently is. ----------> DONE
  Do you remember the operator we use to do "not equal"? */
...
export default function Spinner() {
  ...
  const toggleSpinner = () => {
    (spinnerOn != false) ? setSpinnerOn(false) : setSpinnerOn(true)
  };
  ...
  return (
    <div className='widget-spinner container'>
      ...
      <button id='toggleSpinner' onClick={toggleSpinner}>
        {(spinnerOn === true) ? 'Spinner On' : 'Spinner Off'}
      </button>
    </div>
  );
}
